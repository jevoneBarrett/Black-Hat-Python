# To see high level information about SYSTEM processes, this script must be executed as an admin
import os
import sys
import win32api
import win32con
...
TIME_STOP_ONE_MINUTE = time.time() + 60
...
def monitor():
    head = 'CommandLine, Time, Executable, Parent, PID, User, Privileges'
    #log_to_file(head)
    c = wmi.WMI()
    process_watcher = c.Win32_Process.watch_for('creation')
    while time.time() < TIME_STOP_ONE_MINUTE:
        try:
